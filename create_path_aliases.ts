...
const parsed_tsconfigs: Map<
	string,
	[ Record<string, string>, string ]
> = new Map()

function remove_trailing(
	target_string: string,
	trailing_string: string
): string {
	return target_string.substring(
		0,
		target_string.length - trailing_string.length
	)
}

export function parse_tsconfig(
	path?: string
): [ Record<string, string>, string ] {
	...
	if (tsconfig?.extends !== undefined) {
		const extends_path = join(
			remove_trailing(path, "tsconfig.json"),
			tsconfig.extends
		)

		const [
			extends_alises,
			extends_directory_root
		] = parse_tsconfig(extends_path)
